...
CRITICAL_OUTPUT_KEYS = (
    "reports/b56gv21/version=1/00_integrity_and_temporal_contract.csv",
    "reports/b56gv21/version=1/01_valid_policy_ranking.csv",
    "reports/b56gv21/version=1/02_point_fidelity.csv",
    "reports/b56gv21/version=1/03_rolling_interval_audit.csv",
    "reports/b56gv21/version=1/04_interval_score_paired_bootstrap.csv",
    "reports/b56gv21/version=1/05_coherence_audit.csv",
    "reports/b56gv21/version=1/README_B56G_V21.md",
    "configs/b56gv21/version=1/06_b56g_v21_decision.json",
    "predictions/b56gv21/version=1/valid_asymmetric_predictions.parquet",
    "predictions/b56gv21/version=1/test_asymmetric_predictions.parquet",
)
...
@dag(
    dag_id="maritime_arrival_flow_asymmetric_calibration_v21",
    description=(
        "B56G-v2.1: preserve B56E points and calibrate P10/P90 tails "
        "independently with short-window rolling gates."
    ),
    schedule=None,
...
def maritime_arrival_flow_asymmetric_calibration_v21():
    ...
    def ensure_trainer() -> dict:
        ...
        if (
            config.get("arrival_flow_asymmetric_calibration_version")
            != "b56g-v2.1-asymmetric-aci-v1"
        ):
            raise RuntimeError("Model Trainer does not expose B56G-v2.1")
    ...
    def recalibrate(trainer: dict) -> dict:
        del trainer
        return request_json(
            f"{MODEL_TRAINER_URL}/v1/recalibrate/b56g-v2-1",
            method="POST",
            payload={
    ...
    def verify_outputs(result: dict) -> dict:
        ...
        for key in CRITICAL_OUTPUT_KEYS:
            ...
            if size <= 0:
                raise RuntimeError(
                    f"Empty B56G-v2.1 output: s3://{GOLD_BUCKET}/{key}"
                )
    ...
    def enforce_and_summarize(result: dict, outputs: dict) -> dict:
        if result.get("status") != "SUCCESS":
            raise RuntimeError(f"B56G-v2.1 service failed: {result}")
        findings = result.get("results", {})
        if findings.get("training_executed") is not False:
            raise RuntimeError("B56G-v2.1 unexpectedly trained a model")
        if findings.get("selection_used_test") is not False:
            raise RuntimeError("B56G-v2.1 used TEST for policy selection")
        if findings.get("integrity_gates_passed") is not True:
            raise RuntimeError("B56G-v2.1 integrity gates failed")
        if findings.get("point_fidelity_passed") is not True:
            raise RuntimeError("B56G-v2.1 altered B56E point forecasts")
        if findings.get("coherence_gates_passed") is not True:
            raise RuntimeError("B56G-v2.1 coherence gates failed")
        allowed = {
            "READY_FOR_PROSPECTIVE_SHADOW",
            "SHADOW_WITH_RECENT_CALIBRATION_WARNING",
        ...
        if findings.get("status") not in allowed:
            raise RuntimeError(
                f"Unknown B56G-v2.1 decision: {findings.get('status')}"
            )
        return {
            "status": "SUCCESS",
            "run_id": result.get("run_id"),
            "decision": findings.get("status"),
            "recent30_gates": findings.get("recent30_gates_passed"),
...
maritime_arrival_flow_asymmetric_calibration_v21()
