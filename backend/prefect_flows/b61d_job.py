...
from prefect_flows.b61d_core import (
    MODEL_VERSION,
    SOURCE_MODEL_VERSION,
    SOURCE_POLICY_VERSION,
    STATE_NAMES,
    add_lead_aware_scores,
    chronological_fit_mask,
    decode_frame,
    event_lead_metrics,
    fit_contextual_hsmm,
...
SOURCE_NAME = "b61d_contextual_hsmm"
DATASET_NAME = "maritime_contextual_hsmm_shadow_v1"
...
OUTPUT_RELATION = "serving.maritime_port_call_hsmm_shadow_v1"
SCORECARD_RELATION = "serving.maritime_hsmm_policy_scorecard_v1"
MODEL_CARD_RELATION = "serving.maritime_hsmm_model_card_v1"
...
def _verify_upstream() -> None:
    with _db_connection() as connection, connection.cursor() as cursor:
        ...
        if int(cursor.fetchone()[0]) <= 0:
            raise RuntimeError("B61B-v2.1 predictions are unavailable")
        cursor.execute(
            f"""
            SELECT COUNT(*)
        ...
        if int(cursor.fetchone()[0]) <= 0:
            raise RuntimeError("B61C-v1.1 shadow decisions are unavailable")
...
def load_source() -> pd.DataFrame:
    frame = _query_frame(
        ...
        SELECT
            ...
            g.overdue_h,
            g.plan_progress_ratio,
            g.time_to_planned_departure_h,
            g.arrival_delay_h,
            g.vessel_history_prior_late_gt3_rate,
            g.known_event_any_24h,
    ...
    if frame.empty:
        raise RuntimeError("B61D source join returned no rows")
    if frame.duplicated(["port_call_id", "landmark_at"]).any():
        raise RuntimeError("B61D source has duplicate port-call landmarks")
    for column in (
...
def _baseline_metrics() -> dict[str, dict[str, Any]]:
    scorecard = _query_frame(
        f"""
        SELECT role, metrics
        FROM {BASELINE_SCORECARD_RELATION}
        WHERE policy_version=%s AND selected=true
        """,
        (SOURCE_POLICY_VERSION,),
    )
    result = {str(row.role): dict(row.metrics) for row in scorecard.itertuples()}
    baseline = _baseline_decisions()
    for role, role_frame in baseline.groupby("evaluation_role", sort=False):
...
def _decision_grid(frame: pd.DataFrame, bucket_hours: int) -> pd.DataFrame:
    ...
    eligible["decision_weight"] = 1.0 / counts.clip(lower=1).astype(float)
    return eligible


def _threshold(value: pd.Series, budget_pct: float) -> float:
    return float(
        value.quantile(1.0 - budget_pct / 100.0, interpolation="higher")
    )
...
def _candidate_replay(
    ...
    fit_mask: pd.Series,
    hsmm_weights: list[float],
    gt3_budgets: list[float],
    gt6_budgets: list[float],
    bucket_hours: int,
) -> tuple[pd.DataFrame, dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for hsmm_weight in hsmm_weights:
        scored = add_lead_aware_scores(decoded, fit_mask, hsmm_weight)
        grid = _decision_grid(scored, bucket_hours)
        ...
        for gt3_budget in gt3_budgets:
            for gt6_budget in gt6_budgets:
                ...
                parameters = PolicyParameters(
                    ...
                    high_score_threshold=_threshold(
                        valid["temporal_priority_score"], gt3_budget
                    ),
                    critical_score_threshold=_threshold(
                        valid["critical_priority_score"], gt6_budget
                    ),
                    watch_score_threshold=_threshold(
                        valid["temporal_priority_score"], min(25.0, 2 * gt3_budget)
                    ),
                )
                ...
                metrics["hsmm_weight"] = hsmm_weight
                metrics["candidate_id"] = (
                    f"HSMM_W{hsmm_weight:g}_GT3_{gt3_budget:g}_"
    ...
    if scorecard.empty:
        raise RuntimeError("B61D candidate scorecard is empty")
    selected = scorecard.sort_values(
        [
            "objective",
            "event_recall_at_least_6h",
            "median_event_lead_h",
            "gt3_precision_lift",
        ],
        ascending=[False, False, False, False],
    ).iloc[0].to_dict()
    scorecard["selected"] = scorecard["candidate_id"].eq(
...
def _selected_replay(
    ...
    scored = add_lead_aware_scores(decoded, fit_mask, float(selected["hsmm_weight"]))
    grid = _decision_grid(scored, bucket_hours)
    parameters = PolicyParameters(
    ...
    for role, role_frame in decisions.groupby("evaluation_role", sort=False):
        ...
        metrics["hsmm_weight"] = float(selected["hsmm_weight"])
        metrics["candidate_id"] = str(selected["candidate_id"])
        metrics["objective"] = _lead_objective(metrics)
        diagnostics.append(metrics)
    return decisions, pd.DataFrame(diagnostics), parameters


def _model_diagnostics(bundle: Any, decoded: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for role, role_frame in decoded.groupby("evaluation_role", sort=False):
        total = len(role_frame)
            ...
            rows.append(
                {
                    ...
                    "state": state,
                    "rows": len(state_rows),
                    "row_pct": 100.0 * len(state_rows) / max(total, 1),
                    ...
                    "mean_dwell_steps": state_rows["hsmm_dwell_steps"].mean(),
                }
            )
...
def _quality_gates(
    ...
    valid = selected_diagnostics.loc[
        selected_diagnostics["role"].eq("VALID_CALIBRATE")
    ].iloc[0]
    baseline_valid = baseline["VALID_CALIBRATE"]
    fit_states = model_diagnostics.loc[
        model_diagnostics["role"].eq("VALID_SELECT")
    ]
    ...
    rows = [
        {"check": "SOURCE_B61C_V11_AVAILABLE", "passed": True, "severity": "CRITICAL", "value": SOURCE_POLICY_VERSION},
        {"check": "HSMM_FIT_CHRONOLOGICAL_VALID_SELECT_EARLY", "passed": True, "severity": "CRITICAL", "value": bundle.fit_cutoff},
        {"check": "POLICY_SELECTED_VALID_CALIBRATE_ONLY", "passed": True, "severity": "CRITICAL", "value": "VALID_CALIBRATE"},
        ...
        {"check": "HSMM_FIT_ROWS_SUFFICIENT", "passed": bundle.fit_rows >= 1_000, "severity": "MODEL", "value": bundle.fit_rows},
        {"check": "HSMM_FIT_CALLS_SUFFICIENT", "passed": bundle.fit_calls >= 300, "severity": "MODEL", "value": bundle.fit_calls},
        {"check": "CONTEXTUAL_TRANSITIONS_SUFFICIENT", "passed": bundle.transition_rows >= 1_000, "severity": "MODEL", "value": bundle.transition_rows},
        {"check": "ALL_HIDDEN_STATES_OCCUPIED", "passed": bool((fit_states["rows"] > 0).all()) and len(fit_states) == len(STATE_NAMES), "severity": "MODEL", "value": int((fit_states["rows"] > 0).sum())},
        {"check": "FINITE_HSMM_OUTPUTS", "passed": bool(np.isfinite(decisions[["hsmm_risk_score", "hsmm_escalation_probability", "temporal_priority_score"]].to_numpy()).all()), "severity": "CRITICAL", "value": None},
        {"check": "VALID_GT3_LIFT_NON_INFERIOR_80PCT", "passed": float(valid["gt3_precision_lift"]) >= 0.8 * float(baseline_valid["gt3_precision_lift"]), "severity": "CHALLENGER", "value": float(valid["gt3_precision_lift"])},
        {"check": "VALID_GT3_RECALL_NON_INFERIOR_90PCT", "passed": float(valid["gt3_recall"]) >= 0.9 * float(baseline_valid["gt3_recall"]), "severity": "CHALLENGER", "value": float(valid["gt3_recall"])},
        ...
        {"check": "FRESH_FORWARD_CONFIRMATION_REQUIRED", "passed": True, "severity": "GOVERNANCE", "value": True},
    ]
    return pd.DataFrame(rows)
...
def _materialize_decisions(frame: pd.DataFrame, run_id: str) -> int:
    ...
    CREATE TABLE IF NOT EXISTS {OUTPUT_RELATION} (
        ...
        hsmm_dwell_steps INTEGER NOT NULL,
        p_state_fluid DOUBLE PRECISION NOT NULL,
        ...
        hsmm_weight DOUBLE PRECISION NOT NULL,
        ...
        state TEXT NOT NULL, previous_state TEXT NOT NULL,
        state_changed BOOLEAN NOT NULL, alert_active BOOLEAN NOT NULL,
        new_alert BOOLEAN NOT NULL, action_code TEXT NOT NULL,
        ...
        target_delay_gt_3h BOOLEAN NOT NULL, target_delay_gt_6h BOOLEAN NOT NULL,
    ...
    with _db_connection() as connection, connection.cursor() as cursor:
        cursor.execute(ddl)
        cursor.execute(
            f"DELETE FROM {OUTPUT_RELATION} WHERE model_version=%s",
            (MODEL_VERSION,),
        )
        ...
        for start in range(0, len(output), 1_500):
            ...
            execute_values(
                cursor,
                f"INSERT INTO {OUTPUT_RELATION} ({column_sql}) VALUES %s",
                records,
                page_size=1_500,
            )
        cursor.execute(
            f"CREATE INDEX IF NOT EXISTS ix_b61d_snapshot ON {OUTPUT_RELATION} "
            "(decision_at DESC, state, rank_in_bucket)"
        )
        cursor.execute(
            f"CREATE INDEX IF NOT EXISTS ix_b61d_call ON {OUTPUT_RELATION} "
    ...
    for row in diagnostics.itertuples(index=False):
        ...
        records.append(
            (
                ...
                float(payload["hsmm_weight"]), float(payload["gt3_budget_pct"]),
                float(payload["gt6_budget_pct"]), float(payload["objective"]), True,
    ...
    with _db_connection() as connection, connection.cursor() as cursor:
        ...
        cursor.execute(
            ...
            CREATE TABLE IF NOT EXISTS {SCORECARD_RELATION} (
                model_version TEXT NOT NULL, candidate_id TEXT NOT NULL,
                role TEXT NOT NULL, hsmm_weight DOUBLE PRECISION NOT NULL,
                gt3_budget_pct DOUBLE PRECISION NOT NULL,
                gt6_budget_pct DOUBLE PRECISION NOT NULL,
                objective DOUBLE PRECISION NOT NULL, selected BOOLEAN NOT NULL,
        ...
        cursor.execute(
            f"DELETE FROM {SCORECARD_RELATION} WHERE model_version=%s",
            (MODEL_VERSION,),
        )
        ...
        cursor.execute(
            f"DELETE FROM {MODEL_CARD_RELATION} WHERE model_version=%s",
            (MODEL_VERSION,),
        )
        cursor.execute(
            f"""
            INSERT INTO {MODEL_CARD_RELATION}
            VALUES (%s,%s,%s,%s,%s,%s,false,false,true,%s,now())
            """,
            (
                MODEL_VERSION, SOURCE_MODEL_VERSION, SOURCE_POLICY_VERSION,
                selected_id, Json(_json_ready(model_card)),
                Json(_json_ready(gates.to_dict("records"))), run_id,
            ),
        )
...
def _start_run(checksum: str) -> str:
    metadata = {
        ...
        "fit_role": "VALID_SELECT_CHRONOLOGICAL_EARLY_70PCT",
    ...
    with _db_connection() as connection, connection.cursor() as cursor:
        cursor.execute(
            ...
            (
                ...
                f"postgresql://maritime/{OUTPUT_RELATION}", checksum, Json(metadata),
            ),
        )
...
def run_b61d(
    ...
    del force  # Every research challenger run is versioned and rebuilt deterministically.
    if bucket_hours not in (3, 6, 12):
        raise ValueError("bucket_hours must be 3, 6, or 12")
    for relation in (
        PREDICTION_RELATION,
        FEATURE_RELATION,
        BASELINE_DECISION_RELATION,
        BASELINE_SCORECARD_RELATION,
        "audit.ingestion_run",
    ):
        if not _relation_exists(relation):
            raise RuntimeError(f"Required B61D relation is missing: {relation}")
    _verify_upstream()
    ...
    if not weights or any(item < 0.0 or item > 1.0 for item in weights):
        raise ValueError("HSMM weights must be in [0,1]")
    if not gt3_values or any(item <= 0.0 or item > 25.0 for item in gt3_values):
        raise ValueError("GT3 budgets must be percentages in (0,25]")
    if not gt6_values or any(item <= 0.0 or item > 25.0 for item in gt6_values):
        raise ValueError("GT6 budgets must be percentages in (0,25]")
    source = load_source()
    checksum = _checksum(source, (weights, gt3_values, gt6_values, bucket_hours))
    run_id = _start_run(checksum)
    try:
        fit_mask, fit_cutoff = chronological_fit_mask(source)
        _update_progress(
            run_id, "FITTING_CONTEXTUAL_EXPLICIT_DURATION_HSMM",
            source_rows=len(source), fit_rows=int(fit_mask.sum()),
            source_calls=source["port_call_id"].nunique(),
        )
        bundle = fit_contextual_hsmm(source, fit_mask, fit_cutoff)
        _update_progress(
            run_id, "DECODING_PORT_CALL_REGIME_SEQUENCES",
            fit_calls=bundle.fit_calls, transitions=bundle.transition_rows,
        )
        decoded = decode_frame(bundle, source)
        decoded_fit_mask = decoded["evaluation_role"].eq("VALID_SELECT") & decoded[
            "landmark_at"
        ].le(bundle.fit_cutoff)
        model_diagnostics = _model_diagnostics(bundle, decoded)
        baseline = _baseline_metrics()
        _update_progress(
            run_id, "SELECTING_LEAD_AWARE_POLICY_ON_VALID_CALIBRATE",
            candidate_count=len(weights) * len(gt3_values) * len(gt6_values),
        )
        ...
        decisions, diagnostics, parameters = _selected_replay(
            decoded, decoded_fit_mask, selected, bucket_hours
        )
        gates = _quality_gates(
            source, bundle, model_diagnostics, diagnostics, baseline, decisions
        )
        critical_passed = bool(
            gates.loc[gates["severity"].isin(["CRITICAL", "MODEL"]), "passed"].all()
        )
        challenger_passed = bool(
            gates.loc[gates["severity"].eq("CHALLENGER"), "passed"].all()
        )
        accepted = critical_passed and challenger_passed
        decision = (
            "READY_FOR_B61D_CONTEXTUAL_HSMM_SHADOW"
            if accepted
            else "B61D_CHALLENGER_NOT_ACCEPTED_KEEP_B61C_V11"
        )
        decision_rows = _materialize_decisions(decisions, run_id)
        model_card = {
            "model_version": MODEL_VERSION,
            "model_family": "SEMI_SUPERVISED_CONTEXTUAL_EXPLICIT_DURATION_HSMM",
            ...
            "max_duration_bins": bundle.max_duration_bins,
            "selected_candidate": selected,
            "baseline_model": SOURCE_POLICY_VERSION,
            "challenger_accepted": accepted,
            "test_role": "REUSED_DIAGNOSTIC_NOT_CONFIRMATORY",
            "limitations": [
                "Hidden states are semi-supervised research regimes, not ground truth operations labels.",
                "VALID_SELECT is reused downstream after upstream model selection.",
                "TEST is diagnostic and cannot confirm production readiness.",
                "Actions remain recommendations and are never executed automatically.",
            ],
        }
        _materialize_model_card(scorecard, diagnostics, selected, gates, model_card, run_id)
        artifacts = {
            "model": _put_bytes(
                f"models/b61d/{OUTPUT_PREFIX}/contextual_hsmm.pkl",
                pickle.dumps(bundle, protocol=pickle.HIGHEST_PROTOCOL),
                "application/octet-stream",
            ),
            "model_card": _put_json(
                f"models/b61d/{OUTPUT_PREFIX}/model_card.json", model_card
            ),
        }
        reports = {
            "candidate_scorecard": scorecard,
            "selected_diagnostics": diagnostics,
            "hidden_state_diagnostics": model_diagnostics,
            "quality_gates": gates,
        }
        for name, report in reports.items():
            _put_csv(f"reports/b61d/{OUTPUT_PREFIX}/{name}.csv", report)
        metadata = {
            "decision": decision,
            "model_version": MODEL_VERSION,
            ...
            "selected_hsmm_weight": float(selected["hsmm_weight"]),
            ...
            "challenger_gates_passed": challenger_passed,
            "challenger_accepted": accepted,
            ...
            "artifacts": artifacts,
            "next_block": (
                "B61D_FRESH_FORWARD_HSMM_SHADOW_MONITORING"
                if accepted
                else "KEEP_B61C_V11_AND_REFINE_HSMM"
            ),
        }
        _put_json(f"configs/b61d/{OUTPUT_PREFIX}/final_decision.json", metadata)
...
def verify_b61d_result(result: dict[str, Any]) -> dict[str, Any]:
    required = {
        "decision", "model_version", "decision_rows", "selected_candidate_id",
        "fit_role", "selection_role", "test_role",
    ...
    if missing:
        raise ValueError(f"B61D result misses fields: {missing}")
    if result["selection_role"] != "VALID_CALIBRATE":
        raise ValueError("B61D selection must use VALID_CALIBRATE")
    if result["test_used_for_fit_or_selection"]:
        raise ValueError("B61D TEST leakage contract violated")
    if result["test_role"] != "REUSED_DIAGNOSTIC_NOT_CONFIRMATORY":
        raise ValueError("B61D must disclose TEST reuse")
    if result["production_promotion_allowed"] or result["automatic_action_allowed"]:
        raise ValueError("B61D cannot promote or execute actions")
    if int(result["decision_rows"]) <= 0:
        raise ValueError("B61D produced no shadow decisions")
    return result
