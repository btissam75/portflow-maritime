...
AUDIT_SOURCE = "b61d_v13_dual_stage_policy"
DECISION_TABLE = "serving.maritime_port_call_dual_stage_shadow_v13"
SCORECARD_TABLE = "serving.maritime_dual_stage_scorecard_v13"
MODEL_CARD_TABLE = "serving.maritime_dual_stage_model_card_v13"
...
def _get_pool() -> SimpleConnectionPool:
    ...
    if _pool is None:
        ...
        _pool = SimpleConnectionPool(
            1, int(os.getenv("B61D_V13_DB_POOL_SIZE", "4")), dsn=dsn
        )
    return _pool
...
class DualStageStatus(BaseModel):
    audit_status: str
    decision: str
    policy_version: str
...
router = APIRouter(
    prefix="/api/v1/maritime/dual-stage",
    tags=["Maritime dual-stage shadow policy"],
)


@router.get("/status", response_model=DualStageStatus)
def status() -> DualStageStatus:
    with _connection() as connection, connection.cursor() as cursor:
        ...
        row = cursor.fetchone()
    if row is None:
        raise HTTPException(status_code=404, detail="B61D-v1.3 audit unavailable")
    metadata = dict(row[1])
    return DualStageStatus(
        audit_status=row[0],
...
def model_card() -> dict[str, Any]:
    with _connection() as connection, connection.cursor() as cursor:
        ...
        row = cursor.fetchone()
    if row is None:
        raise HTTPException(status_code=404, detail="B61D-v1.3 model card unavailable")
    return {
...
def scorecard(role: str = Query(default="VALID_CALIBRATE")) -> list[dict[str, Any]]:
    ...
    with _connection() as connection, connection.cursor() as cursor:
        cursor.execute(
            f"""
            SELECT candidate_id,role,early_mode,early_min_score,early_top_k,
                   critical_top_k,hold_windows,objective,passes_constraints,
                   selected,metrics
    ...
    return [
        {
            "candidate_id": row[0], "role": row[1], "early_mode": row[2],
            "early_min_score": row[3], "early_top_k": row[4],
            "critical_top_k": row[5], "hold_windows": row[6],
            "objective": row[7], "passes_constraints": row[8],
            "selected": row[9], "metrics": dict(row[10]),
        }
        for row in rows
    ]
