...
const CapacityEtaBandChart = ({ data, fallback, height = 210 }: CapacityEtaBandChartProps) => {
  const points = useMemo(() => {
    ...
    return source
      .sort(
        (left, right) =>
          new Date(left.decision_at).getTime() - new Date(right.decision_at).getTime(),
      )
      .slice(-36);
  }, [data, fallback]);
  ...
  const option = useMemo(
    () => ({
      ...
      legend: {
        ...
        itemWidth: 15,
        itemHeight: 6,
        data: ['Estimation basse', 'Temps estimé', 'Estimation haute', 'Fourchette prévue'],
      ...
      series: [
        ...
        {
          name: 'Fourchette prévue',
          type: 'line',
          stack: 'eta-confidence',
          silent: true,
          showSymbol: false,
          data: points.map((item) =>
            Number(Math.max(0, item.remaining_p90_h - item.remaining_p10_h).toFixed(1)),
          ),
          lineStyle: { opacity: 0 },
